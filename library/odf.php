...
class Odf
{
    ...
    public function __construct($filename, $config = array())
    {
        ...
        if (($this->manifestXml = $this->file->getFromName('META-INF/manifest.xml')) === false) {
            throw new OdfException("Nothing to parse - check that the manifest.xml file is correctly formed");
        }
     ...
     * @return odf
     */
    public function setImage($key, $value, $w)
    {
...
IMG;
    ...
    private function _save()
    {
        ...
        $lastpos=strrpos($this->manifestXml, "\n", -15);
       	...
       	{
       	$manifdata = $manifdata.'<manifest:file-entry manifest:media-type="image/jpeg" manifest:full-path="Pictures/'.$val.'"/>'."\n";
        }
